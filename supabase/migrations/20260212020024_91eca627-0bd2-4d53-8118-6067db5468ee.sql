...
-- 1. Add restrictive DELETE policy on mining_sessions (matching mining_transactions pattern)
CREATE POLICY "Mining sessions cannot be deleted"
ON public.mining_sessions
FOR DELETE
USING (false);

-- 2. Replace add_mining_reward with rate-limited version
CREATE OR REPLACE FUNCTION public.add_mining_reward(_amount numeric, _block_num integer)
 RETURNS void
 LANGUAGE plpgsql
...
BEGIN
  ...
  -- Rate limiting: enforce minimum interval between blocks
  _min_interval := CASE _current_tier
    WHEN 'bronze' THEN INTERVAL '3 seconds'
    WHEN 'silver' THEN INTERVAL '2 seconds'
  END;

  SELECT MAX(created_at) INTO _last_block_time
  FROM mining_transactions
  WHERE user_id = auth.uid();

  IF _last_block_time IS NOT NULL AND
     (now() - _last_block_time) < _min_interval THEN
    RAISE EXCEPTION 'Mining too fast';
  END IF;
